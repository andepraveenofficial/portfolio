...
### Emmet
Emmet is a tool that helps developers write and edit code more quickly. 
It provides a collection of shortcuts that can be expanded into full blocks of code with a simple command.
...
We can run JavaScript in HTML using the HTML `<script>` element. It is used to include JavaScript in HTML.
Here the __type__ attribute specifies the type of the `<script>`.
To include an external JavaScript file, we can use the HTML `<script>` element with the attribute __src__.
...
* React CDN code is a plain javascript, So ReactJS is javascript library.
* with React CDNs Browser understand the React JS.
...
##### ReactDOM
__ReactDOM__ connect to the __Real DOM__.
...
* ReactJS introduced a new HTML like syntax named JSX to create elements. So, JSX makes life easier.
...
we can easily install any __Node.js__ package or library by using the `npm` command followed by the name of the package.  
...
* `npm` simplifies the process of installing and managing packages in our Node.js projects by providing a centralized repository and a command-line interface to install packages with ease.
...
* **~**: It will update you to all future patch versions but without incrementing the minor version.
...
For example  in `package.json` file:
 ...
 ```js
"devDependencies": {
    "parcel": "^2.10.3" // ^ => Will automatically update to future minor upgrades
}
```

 ```js
"devDependencies": {
    "parcel": "~2.10.3" // ~ => Will automatically update to future patch upgrades
}
```

 ```js
"devDependencies": {
    "parcel": "2.10.3" // No automatic upgrade
}
```
...
##### npx
   ...
   - After Executing `npx parcel index.html` package Automatically two Folders are created .
      * `.parcel-cache`
      * `dist`

##### node_modules
- `node_modules` folder like a cache for the external modules that your project depends upon.
- The `npm install` command is used to recreate the node_modules directory using the `package-lock.json` file.
...
# Ready Made ReactApp Setup
Creating a real-world application involves a lot of setup because a large number of components need to be organised.
1. create-react-app
2. create-vite
...
* There is `File Watcher Algorith` (Written in C++). It keeps track of all the files which are changing realtime & it tells the server to reload.  


### Q: What is `.parcel-cache`?
A `.parcel-cache` is used by parcel(bundler) to reduce the building time.
It stores information about your project when parcel builds it, so that when it rebuilds, it doesn't have to reparse and re-analyze everything from scratch. It's a key reason why parcel can be so fast in development mode.
...
* End of the day any Component is JS Object.
...
A __Higher Order Component__ is function that takes a component and modify some changes and return back as a new component.
...
class App extends Component {
  // Initialization
  constructor(props) {
// `super(props)` is used initialize the properties of Parent when we initialize our component.
...
Example : We have grand, parent, and child components. In this scenario, we want to pass props from the grand to the child. However, the parent component acts as a mediator, receiving props from the grand and passing them to the child.

##### `key` prop in listItem
* A `key` is a special attribute you need to include when creating list of elements in React. Keys are used in React to identify which items in the list are changed, updated, or deleted.
...
![Class Component Life Cycle](./assets/Life%20Cycle.png)
...
##### Mounting
In the Mounting Stage, the instance of a component is created and inserted into the Real DOM.

##### Updating
In Updating Stage, the component is updated whenever there is a change in the component's state. 

##### Unmounting
In Unmounting Stage, the component instance is removed from the DOM. 
...
4. componentDidUpdate => after first render
...
* what we are returning from reducer functions that is stored in state.
...
```js
useEffect(() => {
	console.log("I run every-time when my any change in conditions")
...
##### Own Hook
* we create own hooks for Single Responsibility priciple.
...
so use `<Link><Link/>`component of `react-router-dom`  
...
* React App passes data from Parent to Children. It passes oneway data.
...
Redux is used to create a global central Space.
...
##### dispatching 
`useDispatch` hook is used  dispatch the action. store. 
...
`js-cookies` is a Third Party Package.
...
* So We make small bundles to fast our fast.
* When we need then only load that component.
...
##### lazy
The lazy function in React allows you to import components asynchronously, meaning they are loaded only when needed. 
...
- JS Object is very light-weight.

| **Real DOM**                             | **Virtual DOM**                                |
| ---------------------------------------- | ---------------------------------------------- |
| DOM manipulation is very expensive       | DOM manipulation is very cheap                 |
...
##### Why React is fast?
* React is very fast Because it has `virtual DOM`, `Reconciliation`, `Diff algorithm`. 
...
* `Diff Algorithm` checks difference between two virtual DOMs.
...
* Reconciliation quickly updates it in UI (Real DOM).
...
##### Synthetic Events
* React has its own special way of handling events, and they're called synthetic events.
* These events help your app work smoothly for everyone, even if they're using old browsers.
* They ensure that your app behaves the same way, no matter where it's opened—like a friendly language that all browsers understand.
* Instead of dealing with how each browser talks about events, React simplifies it for you. It's like having a chat with events, and everyone agrees on the same rules, no matter the browser.
Example : Imagine clicking a button
With real DOM events, how it works can vary across browsers. But with synthetic events, it's like clicking a universal button - your code reacts the same way every time, no matter what browser someone's using.

```js 
const handleClick = (event) => {
  // Stop the button from doing its usual thing (e.g., prevent a form from submitting)
  event.preventDefault();// prevents all the default behavior by the browser.

  // Stop the event from telling its parent about the click
  event.stopPropagation();  // prevents the call to the parent component whenever a child component gets called.

  // Now, do your own custom code for the button click
...
```
 ...
 ```js
return(myList.length === 0) ? <Shimer/> : <MyComponent/> 
```

 ```js  
return(myList?.length === 0) ? <Shiimer/> : <MyComponent/> 
```
...
### Optimization
as possible as we decrease the size of our application.
</details>
